# Функция обновляет заметку, принимает словарь,
# ищет по ключу и обновляет значение, заданного ключа
# Функция проверяет даты на правильность ввода,
# по ранее заданному формату
...
def update_note(note):
    while True: # запрашивает ключ для обновления не чувствительный к регистру
        choice_for_update = input(f'Какие данные Вы хотели обновить?{Fore.YELLOW}'
                                  f'(username, title, content, '
                                  f'status, created_date, issue_date)').strip().lower()
        if choice_for_update not in note: # Если ключа нет в словаре выйдет сообщение об ошибке и цикл повторится
            print(f'{Fore.RED}Введите названия корректно.'
                  f'{Fore.RESET}Поля для обновления могут быть:'
                  f'{Fore.YELLOW}{Style.BRIGHT} username, title, content, '
                  f'status, created_date, issue_date')
            continue
        if choice_for_update in ['username', 'title', 'content', 'status']: # если ключ совпадает с одним из значений в условии
            ...
            confirm_for_update = input(f'Вы действительно хотите обновить значение {choice_for_update}'
                                       f'{Fore.YELLOW}{Style.BRIGHT}(да\нет)').strip().lower() # подтверждаем запрос для обновления
            if confirm_for_update == 'да': # если ответ положительный, обновляем значение
                ...
                one_more_update = input(f'Желаете обновить еще одно поле?'
                                        f'{Fore.YELLOW}{Style.BRIGHT}(да\нет)').lower() #Запрашиваем еще одно поле для обновления
        ...
        if choice_for_update in ['created_date', 'issue_date']: # Если ключ относится к дате, то м проверяем в цикле корректность ввода
            ...
            try:
                ...
                continue
            confirm_for_update = input(f'Вы действительно хотите обновить значение '
                                       f'{choice_for_update} '
                                       f'{Fore.YELLOW}{Style.BRIGHT}(да\нет)').strip().lower()
            if confirm_for_update == 'да':
                note[choice_for_update] = dt.strftime(item_for_update,'%d.%m.%Y')
                print(f'{Fore.GREEN}Обновления внесены')
                one_more_update = input(f'Желаете обновить еще одно поле?'
                                        f'{Fore.YELLOW}{Style.BRIGHT}(да\нет)').lower()
                if one_more_update == 'да':
                    ...
                    print(f'Ответ может быть '
                          f'{Fore.YELLOW}{Style.BRIGHT}да/нет. '
                          f'{Fore.RESET}{Style.NORMAL}Обновление завершено')
                    break
            elif confirm_for_update == 'нет':
                print(f'{Style.BRIGHT}Обновления не были внесены.')
                continue
                ...
                print(f'{Style.BRIGHT}Обновления не были внесены.')
                print(f'Ответ может быть {Fore.YELLOW}{Style.BRIGHT}да/нет '
                      f'{Fore.RESET}{Style.NORMAL}Обновление завершено')
